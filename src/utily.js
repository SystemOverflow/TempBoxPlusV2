...
document.getElementById('loading').style.top = (height/2)-80;
document.getElementById('loading').style.left = (width/2)-150;
...
{ 
	...
	var tmp = setInterval(function()
	{
		document.getElementById('loading').style.opacity = x;
		x -= 0.1;
		if(x < 0.0) clearInterval(tmp);
	},50);
	
	document.getElementById("nascondi").disabled = true;
...
function Mostra_c()
{
	...
	var tmp = setInterval(function()
	{
		document.getElementById("loading").style.opacity = x;
		x += 0.1;
		if(x > 0.6) clearInterval(tmp);
	},50);
	document.getElementById("nascondi").disabled = false;
...
function ReSize()
{
	...
	if(Min == a)
	{
			var tempo = setInterval(function()
			{
					...
					document.getElementById("area_testo").size = Min;
					Min++;
					if(Min >= Max) clearInterval(tempo);
			},20);
	}if(Min == Max)
	{
			...
			var tempo2 = setInterval(function()
			{
				...
				document.getElementById("area_testo").size = Min;
				Min--;
				if(Min <= 10) clearInterval(tempo2);
			},20);
	}
}
